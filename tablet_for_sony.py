...
if __name__ == "__main__":
    ...
    for i in range(1,12):
        ...
        frame_num = int(len(mat_data['grayscale'])/(480*640))

        x_value=np.reshape(mat_data['x_value'],(frame_num,480,640)).astype('float')
        y_value=np.reshape(mat_data['y_value'],(frame_num,480,640)).astype('float')
        depth=np.reshape(mat_data['distance'],(frame_num,480,640)).astype('float')
        intensity=np.reshape(mat_data['grayscale'],(frame_num,480,640)).astype('int')
