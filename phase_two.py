...
class PhaseTwo():
    ...
    def _process_file(self, file_num: int, num_files_to_process: int, filename: str, num_ROIs: int,
        ...
        for frame_idx in range(num_frames):
            ...
            if (frame_idx==300):
                roi_name = 'cheek_n_nose'
                roi_bounding_box_pixels = self._get_ROI_bounding_box_pixels(landmarks_pixels, roi_name)
                
                # Draw bounding box for 'cheek_n_nose' ROI
                if roi_bounding_box_pixels.size > 0:
                    cv2.polylines(frame_grayscale_rgb, [roi_bounding_box_pixels], isClosed=True, color=(0, 255, 0), thickness=2)
                    #cv2.putText(frame_grayscale_rgb, roi_name, (roi_bounding_box_pixels[0][0], roi_bounding_box_pixels[0][1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

                # Display the frame with its frame index
                cv2.imshow(f"Frame {frame_idx} with ROI", frame_grayscale_rgb)
                cv2.waitKey(0)  # Wait for a key press to move to the next frame
