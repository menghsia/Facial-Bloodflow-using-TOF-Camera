...
class ProcessHR():
    ...
    def motionComp(self, HRsig, depth):
        ...
        peaks, loc = self.findpeaks(specW)
        ...
        peaks, loc = self.findpeaks(specND)
        maxindex = np.argmax(peaks)
    ...
    def run(self):
        data = sio.loadmat('./Test Data/L_5-25_5.mat')
        tb = data['tb'].flatten()
        bc = data['bc']
        HRsig = data['HRsig'].flatten()
        HRsigRaw = data['HRsigRaw'].flatten()
        I_comp = data['I_comp']
        Depth = data['Depth']
